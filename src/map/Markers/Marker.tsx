...
type StyledBadgeProps = PropsWithChildren<HTMLAttributes<HTMLSpanElement>>

const StyledBadge = ({ children, className = '' }: StyledBadgeProps) => (
  <span
    className={`flex z-20 flex-col absolute -top-2 -right-2 border-2 border-white bg-error text-white rounded-full h-6 w-6 text-xs items-center pt-0.5 ${className}`}
  >
    {children}
  </span>
)
